...
Definition-driven local settings for myday-template. Features contribute **what** appears in Settings (keys, labels, types, defaults). This package owns **persistence**, **validation**, and the **Settings UI**.
...
import com.devindie.myday.feature.settings.api.settingsDestination
...
```bash
./gradlew :domain:jvmTest --tests "com.devindie.myday.domain.usecase.settings.*"
