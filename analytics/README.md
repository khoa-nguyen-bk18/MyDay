...
**Public API:** `com.devindie.myday.analytics.api`  
**Internal impl:** `com.devindie.myday.analytics.impl` (do not import from app code)
...
2. Register your apps:
   - **Android** — package name must match `applicationId` (e.g. `com.devindie.myday`).
   - **iOS** — bundle ID must match the Xcode target (e.g. `com.devindie.myday`).
...
```kotlin
import com.devindie.myday.analytics.api.compose.TrackScreen
...
3. Enable debug mode on the device/emulator:
   ...
   ```bash
   adb shell setprop debug.firebase.analytics.app com.devindie.myday
   ```
