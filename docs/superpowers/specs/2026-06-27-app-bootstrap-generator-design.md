...
| Pascal / product | `MyDay` / `MyDay` |
| Slug | `myday` |
| iOS bundle | `com.devindie.myday.MyDay` |

## Replacement order

Longest-first text replace in text files:

1. `com.devindie.myday` → new package
2. `MyDay` → Pascal name
3. `MyDay` → Pascal name
4. `myday-template` → `{slug}-template` (Firebase placeholder project id)
5. `myday` → slug
...
3. **Move** Kotlin trees: `**/com/devindie/myday` → `**/{package/path}`
4. **Rename** files whose names contain template tokens (e.g. `MyDayApplication.kt`)
