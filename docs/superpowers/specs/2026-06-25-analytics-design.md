...
Konsist: add `:analytics` as an allowed dependency for `:shared` presentation code. ViewModels may import `com.devindie.myday.analytics..` — same category as logging, not `data`.
...
├── build.gradle.kts
└── src/
    ├── commonMain/kotlin/com/devindie/myday/analytics/
    │   ├── AnalyticsClient.kt
...
| `androidApp/.../MyDayApplication.kt` | `analyticsModule(...)` |
