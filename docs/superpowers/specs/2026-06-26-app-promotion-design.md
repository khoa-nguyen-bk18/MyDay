...
**Do not import** `com.devindie.myday.feature.apppromotion.impl` from host code.
...
Android `AndroidAppPromotionPlatform` receives `Context` via Koin `androidContext()` (set in `MyDayApplication`). iOS resolves the key window's root view controller for presenting the share sheet.
